(function(checkers) {
    checkers.Game = function(canvas) {
        ...
        this.setup = function() {
            ...
            setupBoard();
            setupPieces();
            players = [new checkers.Player(checkers.PLAYER_COLOR[0]), new checkers.Player(checkers.PLAYER_COLOR[1])];
            drawBoard();
        ...
        this.processClick = function(event) {
            ...
            var row = Math.floor(y / checkers.SQUARE_LENGTH);
            var col = Math.floor(x / checkers.SQUARE_LENGTH);
        ...
        var setupBoard = function() {
            board = new Array(checkers.NUM_ROWS);
            for (var row = 0; row < checkers.NUM_ROWS; row++) {
                    board[row] = new Array(checkers.NUM_COLS);
                for (var col = 0; col < checkers.NUM_COLS; col++) {
                    board[row][col] = new checkers.Square(getSquareColor(row, col));
                }
            }
        };

        var setupPieces = function() {
            setupPiecesForRows(checkers.PLAYER_COLOR[0], 5, checkers.NUM_ROWS);
            setupPiecesForRows(checkers.PLAYER_COLOR[1], 0, 3);
        };
            ...
            for (var row = startRow; row < endRow; row++) {
                for (var col = 0; col < checkers.NUM_COLS; col++) {
                    if (isValidCheckerSquare(row, col))
                        board[row][col].piece = new checkers.Piece(color);
                }
            }
        };

        var getSquareColor = function(row, col) {
            return isValidCheckerSquare(row, col) ? checkers.SQUARE_COLOR[0] : checkers.SQUARE_COLOR[1];
        };
        ...
        var select = function(row, col) {
            ...
            board[row][col].highlight(context, row, col, checkers.HIGHLIGHT_COLOR);
        ...
        var highlightMoves = function(moves) {
            ...
            for (var i = 0; i < moves.length; i++) {
                var row = moves[i].toRow;
                var col = moves[i].toCol;
                board[row][col].highlight(context, row, col, checkers.HIGHLIGHT_MOVE_COLOR);
        ...
        var getJumps = function(row, col, color, king) {
            ...
            if (king || color === players[0].color) {
                if (inBounds(row - 1, col - 1) && board[row - 1][col - 1].isOccupied()
                    ...
                    move = new checkers.Move(row, col, row - 2, col - 2);
                ...
                for (var col = 0; col < checkers.NUM_COLS; col++) {
                    ...
                    if (piece && player.color === piece.color) {
                        if (players[0].color === piece.color || piece.king) {
                            if (inBounds(row - 1, col - 1) && !board[row - 1][col - 1].isOccupied())
                                moves.push(new checkers.Move(row, col, row - 1, col - 1));
                            if (inBounds(row - 1, col + 1) && !board[row - 1][col + 1].isOccupied())
                                moves.push(new checkers.Move(row, col, row - 1, col + 1));
                        }
        ...
        var inBounds = function(row, col) {
            return row >= 0 && row < checkers.NUM_ROWS && col >= 0 && col < checkers.NUM_COLS;
        };
